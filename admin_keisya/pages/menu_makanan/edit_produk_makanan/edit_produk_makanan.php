...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $editProductCategory = $_POST['editProductCategory']; // Tambahkan ini untuk mengambil data kategori

    // Tentukan direktori tempat menyimpan gambar
    $uploadDir = './uploads/produk_makanan/';
    ...
    if (!empty($editProductImage)) {
        ...
        if (move_uploaded_file($editProductImageTmpName, $uploadPath)) {
            ...
            $sql = "UPDATE produk_makanan SET product_name = ?, description = ?, price = ?, category = ?, image_url = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssssi", $editProductName, $editProductDescription, $editProductPrice, $editProductCategory, $uploadPath, $editProductId);
        } else {
        ...
        $sql = "UPDATE produk_makanan SET product_name = ?, description = ?, price = ?, category = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>window.location.href = '?q=menu_makanan';</script>";
        exit();
    } else {
