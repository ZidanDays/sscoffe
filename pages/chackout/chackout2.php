...
<div class="container-fluid py-5">
                ...
                <div class="col-md-12 col-lg-6 col-xl-5">
                    ...
                    <div class="row g-4 text-center align-items-center justify-content-center pt-4">
                        <div class="col-12">
                            ...
                            echo '<p class="mb-0">Total: $' . $grandtotal . '</p>';
                            ?>
                        </div>
                    </div>
 ...
 <div class="row g-4 text-center align-items-center justify-content-center  py-3">
                    ...
                    <div class="form-item">
                        <label class="form-label my-3">Upload Bukti Transfer (jpg, jpeg, png)</label>
                        <input type="file" name="transfer_proof" accept=".jpg, .jpeg, .png" class="form-control-file">
                    </div>
                    <div class="row g-4 text-center align-items-center justify-content-center pt-4">
                        <!-- Tombol "Pesan" -->
                        <button type="submit" name="pesanBtn" id="pesanBtn" class="btn border-secondary py-3 px-4 text-uppercase w-100 text-primary btn-addto" style="border-radius: 30px;" <?php if (!isset($_SESSION['username'])) echo 'disabled'; ?>>Pesan</button>
                    </div>
                </div>
            </div>
        </form>
...
<!-- Script untuk menonaktifkan tombol "Pesan" jika input tidak lengkap -->
<script>
    document.addEventListener('DOMContentLoaded', function() {
        var pesanBtn = document.getElementById('pesanBtn');
        var firstNameInput = document.querySelector('input[name="first_name"]');
        var tableNumberSelect = document.querySelector('select[name="table_number"]');

        // Mencegah pengiriman formulir jika input tidak lengkap
        pesanBtn.addEventListener('click', function(event) {
            if (!firstNameInput.value || !tableNumberSelect.value) {
                event.preventDefault();
                alert('Mohon isi Nama dan Nomor Meja.');
            }
        });

        // Aktifkan atau nonaktifkan tombol "Pesan" saat input berubah
        firstNameInput.addEventListener('input', togglePesanBtn);
        tableNumberSelect.addEventListener('input', togglePesanBtn);

        function togglePesanBtn() {
            pesanBtn.disabled = !firstNameInput.value || !tableNumberSelect.value;
        }
    });
</script>
