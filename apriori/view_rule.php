            ...
            <thead class="bg-danger text-white">
                ...
                <?php
                    ...
                    while($row=$db_object->db_fetch_array($query)){
                            ...
                            $keterangan = ($row['confidence'] <= $row['min_confidence'])?"<span class='badge badge-danger'>Tidak Memenuhi</span>":"<span class='badge badge-success'>Memenuhi</span>";
...
// Assuming $db_object is your database object
foreach ($data_confidence as $key => $val) {
    if ($val['lolos'] == 1) {
        ...
        // Insert into the product_recommendations table
        $combinations = explode(', ', $val['kombinasi1']);
        $combinationCount = count($combinations);

        for ($i = 0; $i < $combinationCount; $i++) {
            $combination = $combinations[$i];
            $sqlInsert = "INSERT INTO product_recommendations (combination_1, combination_2, confidence) 
                          VALUES ('$combination', '$val[kombinasi2]', $val[confidence])";

            // Execute the SQL query
            $db_object->db_query($sqlInsert);
        }
            ...
            <thead class="bg-danger text-white">
                ...
                $no=1;
                    while($row1=$db_object->db_fetch_array($query1)){
                            ...
                            echo "<td class='text-center'>".($row1['lolos']==1?"<span class='badge badge-success'>Memenuhi</span>":"<span class='badge badge-danger'>Tidak Memenuhi</span>")."</td>";
                        echo "</tr>";
                        $no++;
                        if($row1['lolos']==1){
    ...
    <table class="table table-bordered" width="100%" cellspacing="0">
            <thead class="bg-danger text-white">
                ...
                while($row2=$db_object->db_fetch_array($query2)){
                        ...
                        echo "<td class='text-center'>".($row2['lolos']==1?"<span class='badge badge-success'>Memenuhi</span>":"<span class='badge badge-danger'>Tidak Memenuhi</span>")."</td>";
                    echo "</tr>";
                    $no++;
                    if($row2['lolos']==1){
            ...
            <thead class="bg-danger text-white">
                ...
                $no=1;
                    while($row3=$db_object->db_fetch_array($query3)){
                            ...
                            echo "<td class='text-center'>".($row3['lolos']==1?"<span class='badge badge-success'>Memenuhi</span>":"<span class='badge badge-danger'>Tidak Memenuhi</span>")."</td>";
                        echo "</tr>";
                        $no++;
                        if($row3['lolos']==1){
