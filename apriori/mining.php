...
function mining_process($db_object, $min_support, $min_confidence, $start_date, $end_date, $id_process){
    ...
    $sql_trans = "SELECT * FROM checkout 
    ...
    while($myrow = $db_object->db_fetch_array($result_trans)){
        $dataTransaksi[$x]['tanggal'] = $myrow['order_date'];
        $item_produk = $myrow['product_name'].",";
        ...
        $produk = explode(",", $myrow['product_name']);
        //all items
	...
	<div class='card shadow mb-4'>
        <div class='card-header py-3'>
            <h6 class='m-0 font-weight-bold text-danger'><i class='fa fa-table'></i> Itemset 1</h6>
        </div>
        <div class='card-body'>";
    ...
    foreach ($item_list as $key => $item) {
        ...
        echo "<td class='text-center'>" . price_format($support) . "</td>";
        echo "<td class='text-center'>" . (($lolos==1)?"<span class='badge badge-success'>Memenuhi</span>":"<span class='badge badge-danger'>Tidak Memenuhi</span>") . "</td>";
        echo "</tr>";
        $x++;
    }
    echo "</table></div></div>";
    ...
    foreach ($itemset1 as $key => $value) {
        ...
        echo "<td class='text-center'>" . price_format($supportItemset1[$key]) . "</td>";
        echo "</tr>";
        $x++;
    }
    echo "</table></div></div>";
    ...
    echo "<div class='card shadow mb-4'>
        <div class='card-header py-3'>
            <h6 class='m-0 font-weight-bold text-danger'><i class='fa fa-table'></i> Itemset 2</h6>
        </div>
        <div class='card-body'>";
    echo "<table class='table table-bordered' width='100%' cellspacing='0'>
            <thead class='bg-danger text-white'>
            ...
            if (!empty($variance1) && !empty($variance2)) {
                if ($variance1 != $variance2) {
                    if(!is_exist_variasi_itemset($NilaiAtribut1, $NilaiAtribut2, $variance1, $variance2)) {
                        ...
                        echo "<td class='text-center'>" . $jml_itemset2 . "</td>";
                        echo "<td class='text-center'>" . price_format($support2) . "</td>";
                        echo "<td class='text-center'>" . (($lolos==1)?"<span class='badge badge-success'>Memenuhi</span>":"<span class='badge badge-danger'>Tidak Memenuhi</span>") . "</td>";
                        echo "</tr>";
                        $no++;
                    }
    ...
    foreach ($itemset2_var1 as $key => $value) {
        ...
        echo "<td class='text-center'>" . price_format($supportItemset2[$key]) . "</td>";
        echo "</tr>";
        $no++;
    }
    echo "</table></div></div>";
    ...
    echo "<div class='card shadow mb-4'>
        <div class='card-header py-3'>
            <h6 class='m-0 font-weight-bold text-danger'><i class='fa fa-table'></i> Itemset 3</h6>
        </div>
        <div class='card-body'>";
    echo "<table class='table table-bordered' width='100%' cellspacing='0'>
            <thead class='bg-danger text-white'>
                ...
                if (!empty($itemset1a) && !empty($itemset1b)&& !empty($itemset2a) && !empty($itemset2b)) {
                    ...
                    if(count($temp_array)>0){
                        ...
                        foreach ($temp_array as $idx => $val_nilai) {
                            ...
                            echo "<td>" . $itemset2 . "</td>";
                            echo "<td>" . $itemset3 . "</td>";
                            echo "<td class='text-center'>" . $jml_itemset3 . "</td>";
                            echo "<td class='text-center'>" . price_format($support3) . "</td>";
                            echo "<td class='text-center'>" . (($lolos==1)?"<span class='badge badge-success'>Memenuhi</span>":"<span class='badge badge-danger'>Tidak Memenuhi</span>") . "</td>";
                            echo "</tr>";
                            $no++;
                        }
    ...
    foreach ($itemset3_var1 as $key => $value) {
        ...
        echo "<td class='text-center'>" . price_format($supportItemset3[$key]) . "</td>";
        echo "</tr>";
        $no++;
    }
    echo "</table></div></div>";
...
function get_count_itemset2($db_object, $atribut1, $atribut2, $start_date, $end_date) {
    $sql = "SELECT COUNT(order_date) AS jml, order_date 
            FROM checkout 
            -- WHERE (produk='$atribut1' OR produk = '$atribut2') 
            WHERE (product_name='$atribut1' OR product_name = '$atribut2') 
...
function get_count_itemset3($db_object, $atribut1, $atribut2, $atribut3, $start_date, $end_date) {
    ...
    $sql = "SELECT COUNT(order_date) AS jml, order_date FROM checkout 
            WHERE (product_name='$atribut1' OR product_name = '$atribut2'  OR product_name = '$atribut3') 
                AND order_date BETWEEN '$start_date' AND '$end_date' 
            GROUP BY order_date
